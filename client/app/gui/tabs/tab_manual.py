    ...
    def __init__(self):
        ...
        for name, min_val, max_val, def_val in parameters:
            ...
            slider.valueChanged.connect(
                lambda val, txt=txt_input: txt.setText(str(val))
            )

            def _sync_slider(text, s=slider, minv=min_val, maxv=max_val):
                """Update slider tanpa mengganggu user yang sedang mengetik '-' atau '.'."""
                try:
                    v = float(text)
                except (ValueError, TypeError):
                    return  # biarkan user lanjut mengetik
                # Clamp ke range
                v = max(minv, min(maxv, v))
                # Blokir signal sementara supaya tidak loop
                s.blockSignals(True)
                s.setValue(int(round(v)))
                s.blockSignals(False)

            txt_input.textChanged.connect(_sync_slider)
