...
class DashboardTab(QWidget):
    def __init__(self):
        ...
        # Chart — ⭐ AXES DIBUAT SEKALI DI SINI
        chart_frame = QFrame()
    ...
    def load_and_refresh_data(self):
        ...
        # ── Chart — ⭐ PAKAI AXES YANG SUDAH DI-CACHE ──
        self.ax1.clear()
        self.ax2.clear()
        self.ax1.set_facecolor('#1a1c1e')
        self.ax2.set_facecolor('#1a1c1e')
        ...
        try:
            self.ax1.plot(df_chrono['tanggal'].astype(str),
                          df_chrono['wt'].astype(float),
                          color='#3498db', marker='o', linewidth=2)
        except Exception:
            pass
        self.ax1.set_title("Tren Tinggi Muka Air (WT)",
                           color='white', fontsize=11, fontweight='bold')
        self.ax1.tick_params(colors='white', labelsize=8, rotation=15)
        self.ax1.grid(True, color='#2c3034', linestyle='--')
        ...
        self.ax2.pie(counts, labels=['Aman', 'Siaga', 'Bahaya'],
                     autopct='%1.1f%%',
                     colors=['#2ecc71', '#f1c40f', '#e74c3c'],
                     startangle=90, textprops=dict(color="white"))
        self.ax2.set_title("Proporsi Status Risiko Lahan",
                           color='white', fontsize=11, fontweight='bold')
